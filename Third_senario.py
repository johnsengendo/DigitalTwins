        ...
        for duration, interval in zip([10], intervals):  # Use the longest duration for the overall test loop
            for j in range(num_runs):
                ...
                for flow_id in range(num_flows):
                    ...
                    thread = threading.Thread(target=run_iperf_flow_with_delay, args=(net.get('h1'), h2_ip, server_port, flow_duration, interval, results_file, start_delay))
                    thread.start()
...
def run_iperf_flow_with_delay(h1, h2_ip, server_port, duration, interval, results_file, start_delay):
    time.sleep(start_delay)  # Delay the start of this flow
    run_iperf_flow(h1, h2_ip, server_port, duration, interval, results_file)  # Call the original function to handle the flow
