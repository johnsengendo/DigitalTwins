...
def burst_signal(h1, h2_ip, server_port, burst_duration, burst_bandwidth):
    # Starting iperf with a higher bandwidth in non-blocking mode
...
def create_linear_topology():
    ...
    with open('iperf_results.txt', 'w') as results_file:
        ...
        threads = [threading.Thread(target=run_iperf_flow, args=(net.get('h1'), h2_ip, 5000 + i, durations[0], intervals[0], results_file)) for i in range(num_flows)]
