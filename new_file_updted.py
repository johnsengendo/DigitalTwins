...
def run_iperf_flow(h1, h2_ip, server_port, duration, interval, results_file, bandwidth):
    """
    Running iperf flow with dynamic bandwidth adjustment.
...
def create_linear_topology():
    ...
    num_flows = 2  # Changed from 1 to 2

    # Opening a file in append mode to write the results
    with open('Increase_in_BW', 'a') as results_file:
        durations = [15, 5]  # Duration for flow 1 and flow 2 respectively
        intervals = [0.5, 0.5]  # Interval for flow 1 and flow 2 respectively
        num_steps = 1  # Number of steps (after each step, the bandwidth is increased by a factor of 10)
        ...
        for duration, interval in zip(durations, intervals):
            ...
            # Calculating dynamic bandwidth for each flow
            bandwidth1 = 10  # Bandwidth for flow 1
            bandwidth2 = 50  # Bandwidth for flow 2
            ...
            for flow_id in range(num_flows):
                server_port = 5000 + flow_id
                if flow_id == 0:  # Flow 1
                    thread = threading.Thread(target=run_iperf_flow, args=(net.get('h1'), h2_ip, server_port, durations[flow_id], interval, results_file, f"{bandwidth1}M"))
                else:  # Flow 2
                    thread = threading.Thread(target=run_iperf_flow, args=(net.get('h1'), h2_ip, server_port, durations[flow_id], interval, results_file, f"{bandwidth2}M"))
                    time.sleep(5)  # Start flow 2 after 5 seconds
                thread.start()
            ...
            results_file.write(f"End of step 1, Bandwidth for flow 1: {bandwidth1} Mbps, Bandwidth for flow 2: {bandwidth2} Mbps\n")
            results_file.write("-----\n")
